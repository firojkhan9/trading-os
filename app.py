...
def render_quick_buy_panel(
    ...
    """
    Unified smart trade panel — Dashboard, Scanner, Stock Score, RS Ranking.
    All trades go through the bucket system (portfolio/capital_engine.py).
    Auto-suggests a bucket based on score + signal + regime.
    ...
    if suggested:
        ...
        st.info(f"{conf_icon} **Suggested: {suggested}** — {reason}")
    ...
    with sel_col:
        selected_bucket = st.selectbox(
            ...
            help=(
                ...
                "Intraday  → same day,     score ≥55, 1+ strategy agrees"
            ),
        )
    with info_col:
    ...
    # ── Score vs bucket threshold warning ─────────
    ...
    if composite_score is not None and composite_score < cfg["min_score"]:
        st.warning(
            ...
            f"Switch to a lower bucket or wait for a stronger signal."
        )
        ...
        if st.button(
            ...
            if result["status"] == "EXECUTED":
                st.success(
                    ...
                    f"Cash left in bucket: ₹{result['cash_left']:,.0f}"
                )
                st.rerun()
            else:
...
with tab1:
    ...
    # Show total capital summary across all 3 buckets
